        ...
        try {
          if($request->ajax()){
              ...
              if ($request->dados != '' || $request->dados != ' ') {
                  ...
                  $id = $request->id;
                  if($request->v==1){
                  $conta =  DB::select('select c.conta_id,c.uuid,c.nome,c.apelido,c.foto,c.genero, if((select genero from contas where contas.conta_id = ?) = c.genero, true, false) as equal_genre, if((select pedido_relacionamento_id from pedido_relacionamentos where (conta_id_pedida = c.conta_id || conta_id_pedinte = c.conta_id) and estado_pedido_relac_id = 2), true, false) as accept_request, if((select count(*) from pages as pag where pag.conta_id_a=? || pag.conta_id_b=?) > 0, true, false) as have_relationship, (select count(*) from pages as pg where pg.conta_id_a=c.conta_id and pg.tipo_page_id=1 || pg.conta_id_b=c.conta_id and pg.tipo_page_id=1)as verify_page, (select count(*) from pages as pg where pg.conta_id_a=? and pg.tipo_page_id=1 || pg.conta_id_b=? and pg.tipo_page_id=1) as relationship from contas as c where  c.tipo_contas_id <> 1 and c.estado_conta_id = 1 and c.nome LIKE "%'.$request->dados.'%" || c.tipo_contas_id <> 1 and c.estado_conta_id = 1 and c.apelido LIKE "%'.$request->dados.'%" limit 6', [$conta_id, $conta_id, $conta_id, $id, $id]);
                }else {
